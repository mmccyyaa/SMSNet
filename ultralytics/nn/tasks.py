...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            if m in {Detect, YOLOEDetect, Segment, Segment26, YOLOESegment, YOLOESegment26, Pose, Pose26, OBB, OBB26, STHead}:
                m.legacy = legacy
        elif m is v10Detect:
            ...
            c1 = ch[f]
            args = [*args[1:]]
        elif m in frozenset({MBFusionA, MBFusionB, MBFusionC}):
            c2 = args[0]
            if c2 != nc:
                c2 = make_divisible(min(c2, max_channels) * width, 8)
